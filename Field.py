        ...
        for (name, field), val in zip(fields, data):
            if field is not None:
                ...
                if isinstance(name, tuple):
                    ...
                    if isinstance(field, NestedField):  # it proccess the special tokens if we dont do this
                        setattr(ex, name, field.preprocess(val, just_return=True))
                    else:
...
class Field(RawField):
    ...
    def build_vocab(self, *args, **kwargs):
        ...
        #     if tok is not None))
        specials = ['<unk>', '<pad>', '<bow>', '<eow>', '<bos>', '<eos>']
...
class NestedField(Field):
    ...
    def preprocess(self, xs, just_return=False):
        ...
        Arguments:
            ...
            just_return: if set set to just return, returns the input without furthur processing
    ...
    def numericalize(self, arrs, device=None):
        ...
        for arr in arrs:
            ...
            if arr == '<bos>' or arr == '<eos>' or arr == '<pad>':  # treat these tokens differently
                input_arr = [self.vocab.stoi[arr]] + [1] * (self.nesting_field.fix_length - 1)
                numericalized.append(torch.tensor([input_arr], dtype=self.dtype, device=device))
            else:
                arr2 = self.nesting_field.pad([arr])
                numericalized_ex = self.nesting_field.numericalize(arr2, device=device)
